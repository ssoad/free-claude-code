...
FIELDS: tuple[ConfigFieldSpec, ...] = (
    ...
    ConfigFieldSpec(
        ...
        "models",
        "select",
        settings_attr="routing_strategy",
        default="static",
        options=("static", "cost-optimized", "latency-optimized", "weighted"),
        description="Strategy for resolving fallback chains: 'static' (left-to-right), 'cost-optimized' (cheapest first), 'latency-optimized' (fastest first).",
    ),
    ConfigFieldSpec(
        "ROUTING_COST_REGISTRY",
        "Cost Registry (JSON)",
        "models",
        "textarea",
        settings_attr="routing_cost_registry",
        ...
        description="JSON dictionary defining the cost per 1M tokens for each route.",
        advanced=True,
    ),
    ConfigFieldSpec(
        "MODEL",
    ...
    ConfigFieldSpec(
        ...
        "Coding Route",
        "models",
        settings_attr="model_coding",
        description="Fallback provider/model route for coding tasks (e.g., qwen2.5-coder).",
    ),
    ConfigFieldSpec(
        "MODEL_REASONING",
        "Reasoning Route",
        "models",
        settings_attr="model_reasoning",
        description="Fallback provider/model route for reasoning tasks (e.g., deepseek-r1).",
    ),
    ConfigFieldSpec(
        "MODEL_RESEARCH",
        "Research Route",
        "models",
        settings_attr="model_research",
        description="Fallback provider/model route for research tasks.",
    ),
    ConfigFieldSpec(
        "MODEL_VISION",
        "Vision Route",
        ...
        description="Fallback provider/model route for vision tasks (e.g., llama-3.2-90b-vision).",
    ),
    ConfigFieldSpec(
        "MODEL_FAST",
        "Fast Route",
        "models",
        settings_attr="model_fast",
        description="Fallback provider/model route for fast/low-latency tasks.",
    ),
    ConfigFieldSpec(
        "MODEL_LOCAL",
        "Local Route",
        "models",
        settings_attr="model_local",
        description="Fallback provider/model route for local offline execution.",
    ),
    ConfigFieldSpec(
        "CHAT_EXPOSED_MODELS",
        "Chat Exposed Models",
        "models",
        settings_attr="chat_exposed_models",
        default="claude-sonnet-4,claude-opus-4,claude-haiku-4,coding,reasoning,research,vision,fast,local",
        description="Comma-separated list of model aliases exposed dynamically in the Chat UI dropdown.",
    ),
    ConfigFieldSpec(
    ...
    ConfigFieldSpec(
        ...
        "Brand Name",
        "branding",
        settings_attr="brand_name",
        default="Aura",
        description="The brand name displayed across the UI (e.g., Aura, Free Claude Code).",
    ),
    ConfigFieldSpec(
        "PROVIDER_RATE_LIMIT",
        "Provider Rate Limit",
